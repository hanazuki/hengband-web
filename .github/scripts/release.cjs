...
function contentType(filename) {
  ...
  if (ext === ".json") return "application/vnd.dev.sigstore.bundle+json";
...
async function getOrCreateRelease(github, repo, tag, prerelease) {
  try {
    return await github.rest.repos.getReleaseByTag({ ...repo, tag });
  } catch (/** @type {any} */ err) {
...
module.exports = async ({ github, context, core }) => {
  ...
  if (release.draft) {
    ...
    await using dir = await fs.opendir(releaseDir, { encoding: "utf-8" });
    for await (const dirent of dir) {
      if (!dirent.isFile()) continue;
      const name = dirent.name;
      if (release.assets.some((a) => a.name === name)) continue;
      await using file = await fs.open(path.join(releaseDir, name));
      /** @type {any} */
